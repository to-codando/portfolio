// ../iares/dist/app/index.js
var createApp = (params) => {
  const hostElement = document.body;
  const appConfig = {};
  const mount = () => {
    params.onMount((appConfig == null ? void 0 : appConfig.context) || hostElement);
  };
  const unmount = () => {
  };
  const setup = (params2) => {
...
var _createSelector = (text) => text.split(/(?=[A-Z])/).join("-").toLowerCase();
var _createId = () => {
  const randomStr = Math.floor((1 + Math.random()) * 65536).toString(16).substring(1);
  return `${randomStr}`;
};
var _bindProps = (element, props, isFactory = true, componentId, selector) => {
  if (!props)
    return;
...
var _createChildrenByObject = (template5, context, componentId, selector) => {
  if (typeof template5 === "string") {
    return context.textContent += template5;
  }
  if (typeof (template5 == null ? void 0 : template5.type) === "function") {
    _createComponent(template5, context);
    return;
  }
  if (typeof (template5 == null ? void 0 : template5.type) === "string") {
    const element = document.createElement(template5.type);
    _bindProps(element, template5.props, false, componentId, selector);
    _createChildren(template5.children, element, componentId, selector);
    context.insertAdjacentElement("beforeend", element);
    return;
  }
  if (Array.isArray(template5)) {
    _createChildrenByArray(template5, context, componentId, selector);
    return;
  }
  if (typeof template5 === "object" && !Array.isArray(template5)) {
    const error = new Error();
    error.stack = `ComponentError:Component is not a named function and must be.
    ${JSON.stringify(template5)}
    `;
    throw error;
  }
};
var _createChildrenByArray = (template5, context, componentId, selector) => {
  template5.forEach((templateItem) => {
    _createChildrenByObject(templateItem, context, componentId, selector);
  });
};
var _createChildren = (template5, context, componentId, selector) => !Array.isArray(template5) ? _createChildrenByObject(template5, context, componentId, selector) : _createChildrenByArray(template5, context, componentId, selector);
var _hasStyles = (selector) => document.querySelector(`style#${selector}`);
var _applyCssContext = (cssText, id) => {
  if (!id)
    return cssText;
  const context = /ctx/g;
  return cssText.replace(context, id);
};
var _bindCssStyles = (styles5, selector, componentId) => {
  if (_hasStyles(selector))
    return;
  const css2 = _applyCssContext(styles5, componentId);
...
var _createEventDrive = (element) => {
  const execute = (handler) => {
    handler(element);
  };
  return { execute };
};
var _createComponent = (template5, context) => {
  var _a, _b;
  if (typeof template5.type !== "function")
    throw new Error("Component is not a named function and must be.");
  const { type: componentFactory, props } = template5;
  ...
  const _updateView = (payload) => {
    ...
    _bindProps(hostElement, template5.props, isFunction, componentId, selector);
    _createChildren(template5.children, hostElement, componentId, selector);
    context.insertAdjacentElement("beforeend", hostElement);
    const child = template5.type({ props: template5.props });
    ...
    if (!(child == null ? void 0 : child.template) && typeof (template5 == null ? void 0 : template5.type) === "function") {
      const childHTM = template5 == null ? void 0 : template5.type({ props: props || {}, state, actions });
      _bindProps(hostElement, template5.props, isFunction, componentId, selector);
      if (!childHTM) {
        hostElement.remove();
        return;
 ...
 ${slotOrigin.textContent}`;
    ...
    _eventDrive.execute((element) => {
      ...
      const _bindCssContext = (element2) => {
        if (!scope.uuid)
          return;
...
var render = (template5, context = document.body) => {
  !Array.isArray(template5) ? _createComponent(template5, context) : template5.forEach((templateItem) => _createComponent(templateItem, context));
};

// ../iares/dist/router/router.js
var router = ({ routes: routes2, context }) => {
  ...
  const _setRouterElement = () => {
    const routerElement = context == null ? void 0 : context.querySelector("router-view");
  ...
  const _loadMainRoute = () => {
    ...
    execute(() => !!(mainRoute == null ? void 0 : mainRoute.start), () => (mainRoute == null ? void 0 : mainRoute.start) && navigate(mainRoute.start), () => ({
      name: "Router Error",
      message: "Start router is not defined and must be."
    }));
  };
  const _getMainRoute = () => _routes.find((route) => !!(route == null ? void 0 : route.start));
  const _getRouteByHash = (hash) => {
    return _routes.find((route) => route.regex.test(hash));
  };
  const _getRouteDefault = () => _routes.find((route) => route == null ? void 0 : route.default);
  const _mountRouteByHash = async (hash) => {
    ...
    route == null ? void 0 : route.mount({ context: _routerElement });
  };
